...
def setup_bot(settings: Settings, database: Database) -> Dispatcher:
    ...
    emoji_button = "😊"
    alpha_display = "💳 Карта Альфа Банк 25ОО Р"
    ...
    async def handle_bank_shortcut(message: Message, state: FSMContext) -> None:
        bank_key = next((k for k, v in BANKS_INFO.items() if v["display"] == message.text.strip()), None)
        if not bank_key:
            await message.answer("Не удалось распознать банк. Попробуй снова.")
            return
    ...
    async def handle_referral_cb(call: CallbackQuery) -> None:
        ...
        await call.message.answer(
            "Приглашай друзей и получай бонусы от нашего бота! Начальная ставка — 600 рублей за друга. "
            "Каждый следующий человек увеличивает её на 200: второй принесёт уже 800, третий — 1000 и так далее. "
            "Не забудьте сказать другу, чтобы он уточнил для администрации, что он от вас.",
